...
async function testCDG(num, url) {
    ...
    try {
        ...
        const articles = [];
        $('.elementor-post__title a, .post-title a, h2 a, h3 a, .actu a, .news a, article a').each((i, el) => {
             const text = $(el).text().trim();
             const href = $(el).attr('href') || '';
             if (text.length > 10) articles.push({ text, href });
        });
        
        if (articles.length > 0) {
            console.log(`[HTML GENERIC SUCCESS] Found ${articles.length} links`);
...
async function run() {
    // Run sequentially to avoid timeouts
